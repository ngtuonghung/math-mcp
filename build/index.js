...
export default function createServer() {
    ...
    mathServer.tool("modPow", "Raises a base to an exponent modulo a nonzero modulus", { base: numericInput, exponent: numericInput, modulus: numericInput }, async ({ base, exponent, modulus }) => {
        const [a, b, m] = integers([base, exponent, modulus]);
        return reply(format(modularExponentiation(a, b, m)));
    });
    mathServer.tool("modInverse", "Calculates the inverse of an integer modulo a nonzero modulus", { value: numericInput, modulus: numericInput }, async ({ value, modulus }) => {
        const [a, m] = integers([value, modulus]);
        return reply(format(modularInverse(a, m)));
    });
    mathServer.tool("factorize", "Factorizes a positive integer of at most 64 bits into primes", { value: numericInput }, async ({ value }) => {
    ...
    mathServer.tool("rotateRight", "Rotates an integer right within an 8, 16, 32, or 64-bit field", rotateInput, async ({ value, bits, count }) => {
        const [number, amount] = integers([value, count]);
    ...
    mathServer.tool("packInteger", "Packs an integer into hexadecimal bytes", packInput, async ({ value, bits, signed, endian }) => {
        const [number] = integers([value]);
        return reply(packInteger(number, bits, signed, endian === "little"));
    });
    mathServer.tool("unpackInteger", "Unpacks hexadecimal bytes into an integer", { value: z.string().min(1), bits: widthOption, signed: z.boolean(), endian: endianOption }, async ({ value, bits, signed, endian }) => {
