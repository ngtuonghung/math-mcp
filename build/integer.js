...
function addFactors(value, factors) {
    if (value === 1n)
        return;
    for (const prime of [2n, 3n, 5n, 7n, 11n, 13n]) {
...
export function factorize(value) {
    if (value <= 1n)
        throw new Error("Expected an integer greater than 1");
    if (value > uint64Max)
        throw new Error("Value must be at most 2^64 - 1");
...
export function rotate(value, bits, count, direction) {
    if (count < 0n)
        throw new Error("Rotation count must be nonnegative");
...
export function unpackInteger(value, bits, signed, littleEndian) {
